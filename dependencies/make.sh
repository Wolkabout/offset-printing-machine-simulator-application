...
INCLUDE_DEPLOY_DIR="$BASE_DIR/include/modbus"
...
if [ ! -f "$LIB_DIR/libmodbus.$LIB_EXTENSION" ]; then
    ...
    ./configure --prefix=$BASE_DIR --includedir=$BASE_DIR/carwash/include  \
                --libdir=$LIB_DIR --disable-tests --enable-static --enable-shared=no
...
mkdir -p $INCLUDE_DEPLOY_DIR

# libmodbus
cp $BASE_DIR/libmodbus/src/*.h $INCLUDE_DEPLOY_DIR
