...
MachineView::MachineView(const Simulator& simulator, QWidget *parent) :
        QFrame(parent),
        ...
        ui(new Ui::MachineView)
{
    ...
    // Adding visual to the center piece
    try
    {
    ...
    {
        qDebug("Couldn't set animation, falling back to static image. (Error : %s)", e.what());
    ...
    ui->feederManage->setText(ui->feederManage->text().split(' ')[0] +
                              " (" + QString::number(simulator.getFeeder()->getCount()) + "/" +
                              QString::number(simulator.getFeeder()->getPercentage() * 100) + "%)");
    ...
    ui->deliverManage->setText(ui->deliverManage->text().split(' ')[0] +
                               " (" + QString::number(simulator.getDelivery()->getCount()) + "/" +
                               QString::number(simulator.getDelivery()->getPercentage() * 100) + "%)");
...
{
    QObject::connect(this, SIGNAL(stateChange(bool)), &m_machineView, SLOT(machineStateChange(bool)),
...
{
    std::shared_ptr<simulator::ComponentMessage> message =
            std::make_shared<simulator::ComponentMessage>(simulator::ComponentMessageType::SEVERE,
                                                          "The Emergency Button was triggered!");
